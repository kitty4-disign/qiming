...
function formatDate(iso: string, locale: string): string {
  if (!iso) return "—";
  try {
    return new Date(iso).toLocaleDateString(locale.startsWith("zh") ? "zh-CN" : undefined, {
      year: "numeric",
      month: "short",
      day: "numeric",
    });
    ...
    <div className="h-screen overflow-y-auto bg-[var(--background)] px-4 py-10 [scrollbar-gutter:stable]">
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-8 text-center text-xs text-[var(--muted-foreground)]">
          {t("Welcome to DeepTutor")} · {t("User Management")}
        </p>
      </div>
      ...
      >
        {confirmTarget && (
          <>
            <div className="flex items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--background)]/50 px-3 py-2.5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-[var(--muted-foreground)]">
                  {confirmTarget.user.role === "admin" ? t("Admin") : t("User")} · {t("joined {{date}}", {
                    date: formatDate(confirmTarget.user.created_at, i18n.language),
                  })}
                </p>
              </div>
            </div>
            <p className="mt-3">
              {confirmTarget.kind === "delete"
                ? t("This permanently removes the account and its assignments. This cannot be undone.")
                : confirmTarget.kind === "promote"
                  ? t("Admins can manage users and assignments, and work in the shared main workspace.")
                  : t("They will lose access to the admin area and switch to their own assigned workspace.")}
            </p>
          </>
        )}
      </ConfirmDialog>
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              >
                {createSubmitting ? t("Creating…") : t("Create user")}
              </button>
            </div>
          </form>
        </div>
